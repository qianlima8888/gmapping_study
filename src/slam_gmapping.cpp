...
//利用录制的bag包 处理bag包数据完成和startLiveSLam()几乎一样的功能
//并且会储存五个错误的数据
...
//发布tf 通过调用publishTransform()函数实现
...
bool SlamGMapping::addScan(const sensor_msgs::LaserScan &scan, GMapping::OrientedPoint &gmap_pose)
{
  ...
  reading.setPose(gmap_pose); //gmap_pose值为观察值 在下面利用粒子滤波对值进行优化
...
void SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr &scan)
{
  ...
  if (addScan(*scan, odom_pose))
  {
    ...
    GMapping::OrientedPoint mpose = gsp_->getParticles()[gsp_->getBestParticleIndex()].pose; //获得优化后的值
